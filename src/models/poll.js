...
export const pollOwner = new Schema({
...
poll.statics = {
                ...
                getUsersPolls: function( userId, callback ) {
                        this.find({ 'owner._id':userId }, { 'question':1,'options':1,'createdAt':1 })
                                .exec( callback );
                },
                saveNewPoll : function ( saveData, callback ) {
                        const { question, options, user } = saveData;
                        
                        this.create({ question : question, options : options , owner: user, createdAt:Date.now() }
                                ,(err, created) => {
                ...
                getPolls: function ( options, callback) {
                        return  this.aggregate([{ $project: { 
                                ...
                                }].concat(options), callback );//(( err, polls ) => {
                                  // if(err) return callback( err, null );
